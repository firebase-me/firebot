...
const listRoles = async (client, category?) => {
  ...
  const Roles = await JsonRW.Read('src/../_roles.json');
  const list = Roles.find((c) => c.id == category)?.roles.map((r) => {
...
const deleteRole = async (client, role, category?) => {
  ...
  await JsonRW.Write('./_roles.json', Roles);
  // delete role
  if (!protectedRoles.includes(role)) await client.guilds.cache.get(devServer).roles.cache.get(role).delete();
};
...
const deleteCategory = async (client, category, bias?) => {
  ...
  // delete category
  await JsonRW.Write('./_roles.json', Roles);
  if (bias) if (!protectedRoles.includes(category)) await client.guilds.cache.get(devServer).roles.cache.get(category).delete();
};
...
const purgeRoles = async (client, confirmation) => {
  ...
  const roles = await client.guilds.cache.get(devServer).roles.cache.filter((r) => !usedRoles.has(r.id));
