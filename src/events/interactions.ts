...
client.on("interactionCreate", async interaction => {
    ...
    selected.options.forEach(t => {
        options.push({
                label: t,
                description: 'Toutes les news du Discord Les Moddeurs Français',
                value: t
            }      );
    });
    ...
    // reply
    const xx = await interaction.editReply({ embeds: [embed], components: [row], ephemeral: true });
    ...
    collector.on('collect', async i => {
        ...
        if(code != selected.correct) {
            ...
            await interaction.member.roles.add(config.get('discord.roles.verified')).catch(err => console.log(`No permissions to assign roles!`));
        }
    });
});
}
